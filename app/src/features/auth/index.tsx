...
import { useEmailSubmit } from "./use-auth";
import { useState } from "react";
import { AuthStrategy } from "./types";

export default function AuthForm() {
  const [formStrategy, setFormStrategy] = useState<AuthStrategy>("login");
  const {
    email,
    setEmail,
    isLoading,
    ...
    handleKeyPress,
    validateEmail,
  } = useEmailSubmit(formStrategy);

  const toggleStrategy = () => {
    setFormStrategy(prev => prev === "login" ? "register" : "login");
  };
      ...
      <div className="flex flex-col gap-2">
        ...
        >
          {isLoading ? (
            ...
            formStrategy === "login" ? "Войти" : "Зарегистрироваться"
          )}
        </Button>
